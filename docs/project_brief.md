...
- LLM: Gemini 3 Flash Preview via AI Studio (paid tier, api_key auth)
- TTS: Gemini 3.1 Flash TTS Preview
- Embeddings: Gemini `gemini-embedding-001` (768-dim Matryoshka, same paid-tier api_key path)
...
| 2026-06-30 | **Gemini auth (LLM + embedding): ADC/Vertex → api_key/AI Studio.** The same-day BGE → Vertex AI deploy uncovered `DefaultCredentialsError` on every `/create` call: Render isn't a GCP environment, ADC was never bootstrapped there, and the previous OOM-at-import had masked the failure. Considered Option A (provision a GCP service-account key as a Render Secret File and keep ADC) and Option B (switch both clients to api_key/AI Studio). Picked B. The 2026-06-13 policy that mandated ADC/Vertex was protecting against AI Studio's free-tier "may train on prompts" terms; Aarva is on the paid tier, whose terms explicitly don't train, so the data-governance constraint holds under api_key too. Code: `VertexAIEmbeddingClient` renamed to `GeminiEmbeddingClient` with both `auth_mode='api_key'` and `auth_mode='adc'` supported (vectors are interchangeable across modes — same model, same task types, same dimensions). pipeline.yaml flipped on both `llm.auth_mode` and the embedding block. The old `provider: vertex_ai` name is kept as a factory alias for backwards-compat. | Avoids the SA-key rotation burden on Render entirely (no JSON file to leak, no rotation cadence to maintain). Single auth model across laptop + Render. Reuses the `GEMINI_API_KEY` env var that's already configured. Marginal downside: api_key path goes through AI Studio's RPM caps, which are tighter than Vertex's paid-tier ceilings — fine for Aarva's volume but worth re-checking if traffic 10× from here. | Yes — `auth_mode: adc` is a one-line YAML edit on both blocks; `gcp_project` + `gcp_location` were intentionally left populated so the flip doesn't require a re-population. No vector re-embed needed in either direction. |
