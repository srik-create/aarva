...
1. **Bluesky reverse-lookup (trend-signal v2 concept B).** Full spec
   at `docs/session_plan_trend_signal_v2.md`. Bluesky's `searchPosts`
   endpoint now requires authentication (confirmed 2026-08-20 —
   Bluesky's `getTrends`, the forward-source endpoint, is unaffected
   and ships regardless). The user is setting up a dedicated Bluesky
   bot account + app password themselves (Claude Code can't get past
   the signup CAPTCHA + email verification). Reverse lookup shipped
   2026-08-20 with HN only in the meantime — see "Recently completed".
   **Trigger: user confirms the bot account + app password exist —
   check for this at the start of the next session before assuming
   it's still pending.**

2. **Hook prompt rewrite (Stage 8a).** Current hooks are
...
4. **Listen-test the TTS audio settings.** 2026-06-26 shipped:
...
5. **Retroactive audio-quality pass on older episodes.** Anything
...
- **Trend-signal v2 PR 2 — Bluesky + HN as new forward trend
  sources, plus HN-only reverse lookup.** Full spec at
  `docs/session_plan_trend_signal_v2.md`, concepts A + B. Scope
  narrowed by real rule 6a verification before any of it was built:
  Reddit dropped entirely (OAuth closed November 2025, the
  unauthenticated `.json` fallback the spec relied on shut down May
  30, 2026 — confirmed dead, no viable path); Bluesky's `searchPosts`
  now also requires authentication (confirmed mid-2026) and is
  deferred — see the "Deferred" section's Bluesky reverse-lookup
  entry. `getTrends` (the forward-source endpoint) is unaffected and
  ships regardless.
  - **New forward sources**: `TrendSource` gained a `kind` field
    (`google_trends` | `bluesky_trends` | `hn_frontpage`, defaulting
    to `google_trends` for backward compat).
    `aarva/sources/trend_crawler.py` refactored into a per-kind
    dispatch table so the shared translate+insert loop doesn't need
    to know which source produced a trend. Bluesky's `getTrends`
    (skips `stale` AND a real-but-undocumented `saturating` status
    found during verification — not in the spec's assumed vocab —
    plus any category outside the configured allowlist; politics
    still gets crawled for visibility in `raw_metadata_json`, just
    never reaches the matcher) and HN's `search_by_date` (points
    threshold enforced server-side, verified live) both added.
  - **Reverse lookup (HN only)**: new
    `aarva/services/article_virality.py` and `article_virality_hits`
    table. Scans already-scored articles (JTBD filter only — locked
    asymmetric decision: NO age minimum, since external virality is
    trusted signal and Aarva's editorial bar already ran when the
    article was first scored; the 90-day scan window is a COST cap,
    not an editorial guardrail) against HN Algolia's URL-search.
    HN's `query` param does fuzzy text matching, not exact URL
    equality (verified live — a search for one URL also matched a
    same-domain-different-path variant), so results are filtered
    client-side to the exact `canonical_url`. Idempotent per
    `(article_id, source_name, external_url)` — a persistent external
    post is recorded once, ever, not re-inserted on every scan.
    Folds into the same `--stage 3` invocation as the forward
    crawl+match (no separate stage or flag — same 2026-08-20 decision
    as the sources themselves).
  - **Review CLI**: new "Trending Aarva articles" section (mirrors
    the existing "Trending topics" section) with `vNa`/`vNd` batch
    commands parsed in the same input line as piece/trend commands.
    Adds land as `review_status='approved'` (per the 2026-08-15
    auto-approve fix — the `vNa` keystroke is itself the approval).
  - **No new `pipeline.yaml` toggle flags anywhere in this PR** — per
    user decision 2026-08-20: Bluesky/HN sources use the SAME
    per-source `enabled: true/false` field `trend_sources.yaml`
    already has (a parallel flag would repeat the exact
    redundant-toggle mistake already corrected once for the old
    blanket `trends.enabled`).
  - **Verified for real at every layer**: a live crawl across all 5
    sources (3 Google Trends regions + Bluesky + HN) found 51 real
    trends with zero failures — Bluesky correctly filtered out
    politics (7/10 of a raw sample were political) while HN returned
    clean, high-quality stories. The real reverse-lookup scan ran
    against a full disposable copy of production data (1,690 real
    candidate articles) and found 2 genuine Aarva articles currently
    on Hacker News with correct scores (101 and 297 points) — exactly
    the spec's own suggested end-to-end verification. The full
    interactive review flow was driven with simulated stdin against
    a fixture with a normal piece, a trend match, and a virality hit
    all in one command line — all three landed correctly (the
    virality-hit article got `slot='bonus'` since its JTBD wasn't
    `delight`, matching the trend-matcher's own slot convention).
  - 26 new tests (repo total: 169) — Bluesky status/category
    filtering, HN parsing, unknown-source-kind handling, the
    reverse-lookup scan's guardrail SQL (including the "no age
    minimum" asymmetric decision), HN's exact-URL client-side filter,
    end-to-end scan idempotency, and the review CLI's new `vN`
    token parsing + apply logic.
